...
- **Jardinets.** Fold 8 `samsung-galaxy-fold-8-mupi`, Tab A11 `samsung-galaxy-tab-a11-mupi` (SM-X130) y DGX `dgx-spark`, circuito `samsung-galaxy-fold-8`, tema música (`control/playlist?screen=<pantalla>-tema`, publicado con `tools/publica-playlists-kioskos.py`). Cada dispositivo conserva su `screen` para telemetría; el gemelo CanalKiosk manda `play<num>` al circuito y llega a los tres. Contrato general: https://admira.tv/mcp/player-contract.md
