...
- **Jardinets.** Fold 8 `samsung-galaxy-fold-8-mupi`, Tab A11 `samsung-galaxy-tab-a11-mupi` (SM-X130) y DGX `dgx-spark`, circuito `samsung-galaxy-fold-8`, tema música. El DGX conserva su `screen` para telemetría pero abre con `leader=samsung-galaxy-fold-8-mupi-tema`, de modo que consume el tema canónico del Fold y cada edición de CanalKiosk se refleja automáticamente; `dgx-spark-tema`, publicado con `tools/publica-playlists-kioskos.py`, queda como respaldo. El gemelo CanalKiosk manda `play<num>` al circuito y llega a los tres. Contrato general: https://admira.tv/mcp/player-contract.md
