...
(function(root){
 async function findPath({start,goal,load,distance,signal,onProgress=()=>{},maxNodes=300,maxDistance=1000,arrive=node=>node.id===goal.id}){
  ...
  while(open.length){
   ...
   if(arrive(nodes.get(cur.id))){const path=[cur.id];while(parents.has(path[0]))path.unshift(parents.get(path[0]));return {panos:path,nodes:path.map(id=>nodes.get(id))};}
   ...
   for(const link of node.links){
    ...
    cost.set(next.id,g);parents.set(next.id,cur.id);open.push({id:next.id,g,f:g+1.6*distance(next.position,goal.position)});
   }
