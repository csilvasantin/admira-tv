...
(function(root){
 ...
 function create({getSurface,prepare,getCamera,onState=()=>{},onCancel=()=>{},afterPaint=fn=>requestAnimationFrame(()=>requestAnimationFrame(fn)),timeoutMs=15000}){
  ...
  function cancel(reason='cancel',requestId){
   ...
   const run=active;active=null;clearTimeout(run.timer);onCancel(run.requestId);emit(run,'cancelled',reason);return true;
  }
  ...
  function fail(run,reason){if(active!==run)return;clearTimeout(run.timer);active=null;onCancel(run.requestId);emit(run,'error',reason)}
  function check(){
   ...
   run.painting=true;
   afterPaint(()=>{if(active!==run||disposed)return;run.painting=false;if(!['ready','unavailable'].includes(run.scene?.status)||run.scene?.pano!==run.surface.pano||!cameraMatches(run))return;run.ready=true;clearTimeout(run.timer);emit(run,'ready')});
  }
