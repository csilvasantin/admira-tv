...
function fixture(embedded=true,walkMode=false) {
  const listeners={},sent=[],calls={boot:0,refresh:0,stop:0,hide:0,walk:0,surfaceCancel:0,surfaceFocus:[],manual:0,routeStart:[],routeSpeed:[],routeCancel:0,audio:[],mute:0};
  ...
  const context=vm.createContext({window:{OUTDOOR_EMBED:embedded,parent},location:{origin:'https://admira.tv'},OutdoorContext,OutdoorSites,DoohSurfaces:require('../../js/dooh-surfaces.js'),sv:{getPano:()=>null},activeWalkSite:OutdoorSites.get('vila'),bootWalk:async()=>calls.walk++,walkLoadFailure:()=>{},
    ...
    routeWalk:{setSpeed:(...args)=>calls.routeSpeed.push(args),cancel:()=>calls.routeCancel++,start:(...args)=>calls.routeStart.push(args)},screenAudio:{mute:()=>calls.mute++,command:c=>calls.audio.push(c)},
    playerStockAbort:null,remoteSession:null,closePlayerRemote:()=>{},screenPlayers:new Map(),handlePlayerCommand:p=>(calls.player??=[]).push(p),streetWalk:null,WALK_MODE:walkMode,walkBootTimer:null,walkBootFailed:false,walkBootGeneration:0,clearTimeout,stopCam:()=>calls.stop++,hideStreetView:()=>calls.hide++,fmtHm:()=> '09:45',FRANJAS:[{aforo:460}],selFranjaIdx:0,_shotDone:false});
...
test('generic inspection requests the current photographic surface without a walk command',()=>{const f=fixture(true,true);f.dispatch(OutdoorContext.message('walk-command',{action:'inspect'}));assert.equal(f.calls.inspect,1);assert.equal(f.calls.walk,0);assert.equal(f.calls.manual,0);});

test('an active speed message adjusts the route without restarting, cancelling or muting again',()=>{const f=fixture(true,true);f.dispatch(OutdoorContext.message('route-command',{action:'speed',routeId:'vila-jardinets',requestId:4,speed:8}));assert.deepEqual(f.calls.routeSpeed,[[8,4,'vila-jardinets']]);assert.equal(f.calls.routeStart.length,0);assert.equal(f.calls.routeCancel,0);assert.equal(f.calls.surfaceCancel,0);assert.equal(f.calls.mute,0);});

test('remote player commands require the exact parent and navigation stays locked for that session',()=>{const f=fixture(true,true),msg=OutdoorContext.message('player-command',{action:'open',screenId:'vila-left',requestId:12});f.dispatch(msg,{origin:'https://other.example'});f.dispatch(msg,{source:{}});assert.equal(f.calls.player,undefined);f.dispatch(msg);assert.equal(f.calls.player.length,1);f.context.remoteSession={screenId:'vila-left',requestId:12};f.dispatch(OutdoorContext.message('walk-command',{action:'site',siteId:'lesseps'}));f.dispatch(OutdoorContext.message('route-command',{action:'start',routeId:'vila-jardinets',requestId:3}));assert.equal(f.calls.walk,0);assert.equal(f.calls.routeStart.length,0);f.dispatch(OutdoorContext.message('player-command',{action:'close',screenId:'vila-left',requestId:12}));assert.equal(f.calls.player.length,2);});
