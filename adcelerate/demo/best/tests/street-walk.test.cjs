...
class Panorama {
 ...
 getPano(){return this.pano;}getStatus(){return this.status;}getPov(){return this.pov;}getLinks(){return this.links;}getZoom(){return this.zoom;}
...
function fixture(){const p=new Panorama(),requests=[],states=[],changes=[];const service={getPanorama:(request,callback)=>requests.push({pano:request.pano,callback})};
 const walk=Walk.create({panorama:p,service,afterPaint:fn=>fn(),panelPano:'panels',frontPano:'front',onState:s=>states.push(s),onPanoChange:pano=>changes.push(pano)});
...
test('one pending hop, no invented destination and no inherited date or support after leaving panels',()=>{
 const f=fixture();try{f.reply(0);f.p.setPov({heading:136,pitch:2});assert.equal(f.walk.command({action:'forward'}),true);assert.equal(f.walk.getState().date,'');assert.equal(f.walk.getState().supportVisible,false);assert.equal(f.walk.command({action:'forward'}),false);assert.equal(f.requests.length,2);f.reply(1,'2024-07',[{pano:'panels',heading:316,description:'Quiosco'}]);assert.equal(f.walk.getState().pano,'next');assert.equal(f.walk.getState().date,'2024-07');assert.equal(f.walk.getState().steps,1);assert.equal(f.walk.command({action:'link',pano:'invented'}),false);}finally{f.walk.dispose();}
});
test('links_changed cannot inject previous-node links into the new node',()=>{
 const f=fixture();try{f.reply(0);f.walk.command({action:'link',pano:'next'});f.reply(1,'2025-01',[{pano:'panels',heading:316,description:'Back'}]);f.p.links=[{pano:'stale',heading:30}];f.p.fire('links_changed');assert.deepEqual(f.walk.getState().links.map(x=>x.pano),['panels']);}finally{f.walk.dispose();}
});
