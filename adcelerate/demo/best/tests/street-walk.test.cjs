...
test('walking arrival updates the return kiosk without moving camera or resetting walked steps',()=>{const f=fixture();try{f.reply(0);f.walk.command({action:'link',pano:'next'});f.reply(1,'2023-03',[{pano:'third',heading:140}]);const before={pano:f.p.pano,pov:{...f.p.pov},zoom:f.p.zoom,steps:f.walk.getState().steps,requests:f.requests.length};assert.equal(f.walk.setSite({homePano:'jardinets',homePov:{heading:290,pitch:-6,zoom:.9},frontPano:null}),true);assert.deepEqual({pano:f.p.pano,pov:{...f.p.pov},zoom:f.p.zoom,steps:f.walk.getState().steps,requests:f.requests.length},before);assert.equal(f.walk.command({action:'front'}),false);f.walk.command({action:'home'});assert.equal(f.p.pano,'jardinets');assert.equal(f.p.pov.heading,290);f.reply(2);assert.equal(f.walk.getState().steps,1);}finally{f.walk.dispose();}});
test('status OK cannot acknowledge the requested panorama while the SDK location still belongs to the old one',()=>{const f=fixture();try{f.reply(0);f.p.locationPano='panels';f.walk.command({action:'link',pano:'next'});f.reply(1);assert.equal(f.walk.getState().status,'loading');assert.equal(f.walk.getState().steps,0);f.p.locationPano='next';f.p.fire('position_changed');assert.equal(f.walk.getState().status,'ready');assert.equal(f.walk.getState().steps,1);}finally{f.walk.dispose();}});
test('SDK location coherence is checked again after paint and can recover on a later position update',()=>{const paints=[],f=fixture({afterPaint:fn=>paints.push(fn)});try{f.reply(0);paints.shift()();f.walk.command({action:'link',pano:'next'});f.reply(1);f.p.locationPano='panels';paints.shift()();assert.equal(f.walk.getState().status,'loading');f.p.locationPano='next';f.p.fire('position_changed');paints.shift()();assert.equal(f.walk.getState().status,'ready');assert.equal(f.walk.getState().steps,1);}finally{f.walk.dispose();}});
